            ...
            GET: async ({ request }) => {
                ...
                if (code) {
                    try {
                        ...
                        // TODO(human): Add access control check here
                        // Check if user.email is authorized before creating session
                        // Example approaches:
                        // 1. Email whitelist: if (!['admin@example.com', 'user@example.com'].includes(user.email)) { return errorResponse() }
                        // 2. Domain check: if (!user.email.endsWith('@yourcompany.com')) { return errorResponse() }
                        // 3. Database lookup: const isAuthorized = await db.user.findUnique({ where: { email: user.email } })
